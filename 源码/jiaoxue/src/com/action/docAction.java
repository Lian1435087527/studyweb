...
public class docAction extends ActionSupport
{
	...
	public String zuoyeAdd()
	{
		...
		docDAO.save(doc);
		this.setMessage("操作成功");
		this.setPath("zuoyeMana.action");
		return "succeed";
	}
	
	public String zuoyeDel()
	{
		TDoc doc=docDAO.findById(id);
		doc.setDel("yes");
		docDAO.attachDirty(doc);
	...
	public String zuoyeMana()
	{
		String sql="from TDoc where del='no' and type='zuoye'";
		List docList=docDAO.getHibernateTemplate().find(sql);
		Map request=(Map)ServletActionContext.getContext().get("request");
		request.put("docList", docList);
		return ActionSupport.SUCCESS;
